...
export class PigmyAgentPage extends CbsPage {
  ...
  private inp = async (id: string, val: string): Promise<void> => {
    if (!val) return;
    await this.f(id).waitFor({ state: 'visible', timeout: 10_000 }).catch(() => {});
    await this.f(id).fill(val);
    await this.f(id).press('Tab');
  };
  private sel = async (id: string, val: string): Promise<void> => {
    if (!val) return;
    await this.f(id).waitFor({ state: 'visible', timeout: 10_000 }).catch(() => {});
    await this.f(id).selectOption(val).catch(() => {});
  };
  ...
  async create(data: PigmyAgentData): Promise<string> {
