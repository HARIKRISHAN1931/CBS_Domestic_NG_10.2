...
export class BlockmunicipalMasterPage extends CbsPage {
  protected readonly screenId = 'MUNICIPALITYBLOCKMASTER';
  ...
  async save(): Promise<string> {
    ...
    if (await errToast.isVisible({ timeout: 5_000 }).catch(() => false)) throw new Error(`CBS save error: ${(await errToast.innerText().catch(() => '')).trim()}`);
    return this.toast.getSuccess();
  }

  async create(data: BlockmunicipalMasterData): Promise<string> { await this.fillForm(data); return this.save(); }
